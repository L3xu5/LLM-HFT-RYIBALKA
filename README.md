...
**Релиз на GitHub:** при каждом запуске workflow job **github-release** создаёт [**GitHub Release**](https://docs.github.com/repositories/releasing-projects-on-github/about-releases) с автозаметками и архивом **`rybalka-web-<тег>.zip`**. Имя релиза и git-тега: при **push тега `v*`** это сам тег (например `v1.0.0`); при **Run workflow** или ином событии без тега используется **`ci-<номер_прогона>`** (уникально для запуска). **attach-android-apk** добавляет **`rybalka-<тег>.apk`** после **EAS** (профиль **`production-apk`** в [`eas.json`](eas.json)). Если Android-сборка падает, релиз с **web zip** остаётся без APK. Теговый релиз: `git tag -a v1.0.0 -m "..." && git push origin v1.0.0`.
